...
int custom_bcast(
    ...
    if(MPI_Comm_rank(comm, &rank) == MPI_ERR_COMM) return MPI_ERR_COMM;

    if(!buffer) return MPI_ERR_BUFFER;

    if(rank == root){
        //transmite para todos no canal e bloqueia
        if(MPI_Comm_size(comm, &n_tasks) == MPI_ERR_COMM) return MPI_ERR_COMM;

        if(root >= n_tasks) return MPI_ERR_ROOT;

        for(int i = 0; i < n_tasks; i++)
            if(i != root){
                int mpi_send_retval = MPI_Send(buffer, count, datatype, i, 0, comm);
                if(mpi_send_retval) return mpi_send_retval;
            }
    }
    else{
        MPI_Status status;
        int mpi_recv_retval = MPI_Recv(buffer, count, datatype, root, MPI_ANY_TAG, comm, &status);
        if(mpi_recv_retval == MPI_ERR_RANK) return MPI_ERR_ROOT;
        if(mpi_recv_retval) return mpi_recv_retval;
    }
    return MPI_SUCCESS;
}
