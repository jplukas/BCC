...
import Tokenizer (isNumeral, isSymbol, isReservedWord, isNumeralChar)
...
analyze tree = case tree of
  ...
  Pair first rest -> case first of
    ...
    -- Leaf "of" ->  OfS (analyzePos 1) (analyzePos 2)
    ...
    where
      ...
      getSymbol i = case tree `index` i of
        Leaf symbol -> if (not ((isValidIdentifier symbol))) then 
          error "ERRO analyze: identificador inválido" 
          else symbol
        _ -> error "ERRO analyze: símbolo esperado no lugar de uma expressão"

isValidIdentifier id = not (isReservedWord id || isNumeralChar (head id))
